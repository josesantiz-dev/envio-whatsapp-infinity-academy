...
function obtenerListaNegra() {
    ...
    if (listaNegra == null || listaNegra == "[]") {
        listaNegra = [];
    } else {
        listaNegra = JSON.parse(listaNegra);
    }
    return listaNegra;
}
                    ...
                    participantes.forEach(async participante => {
                        ...
                        setTimeout(async function () {
                            let promise = new Promise(async (resolve) => {
                                ...
                                if (listaNegra.includes(numeroTelefono)) {
                                    resolve({
                                        estatus: false,
                                        data: response
                                    })
                            ...
                            if(respuesta.estatus == true){
                                if (respuesta.data.data.error) {
                                    //Mandar numero a lista negra
                                    let listaNegraActual = obtenerListaNegra();
                                    if (listaNegraActual.length > 0) {
                                        let isExiste = listaNegraActual.filter(x => x == numeroTelefono);
                                        if (isExiste.length > 0) {
    
                                        } else {
                                            listaNegraActual.push(numeroTelefono);
                                        }
                                    }
                                    errores += 1;
                                    document.getElementById(idTdErrores).innerHTML = errores;
